  ...
  switch (setups) {
    ...
    case 0 :
      ...
      if (int(firebase.connect()) == 1) {
        Serial.println("Create SafeBox");
        firebase.setStr("boxes/" + mac_address + "/boxName", SafeBoxName);
        firebase.setStr("boxes/" + mac_address + "/boxPassword", SafeBoxPassword);
        Serial.println("Create SafeBoxUsers");
        firebase.setStr("boxes/" + mac_address + "/users/aaa@aaa", "admin");
        firebase.setStr("boxes/" + mac_address + "/users/bbb@bbb", "viewer");
        Serial.println("Create SafeBoxConfig");
        firebase.setInt("boxes/" + mac_address + "/setinterval", 50);
        firebase.setInt("boxes/" + mac_address + "/cb_motion", 60);
        firebase.setInt("boxes/" + mac_address + "/cb_temp", 50);
        firebase.setInt("boxes/" + mac_address + "/mute_buzzer", 0);
        firebase.setInt("boxes/" + mac_address + "/mode_buzzer", 0);
        firebase.setInt("boxes/" + mac_address + "/cb_temp_buzzer", 100);
        firebase.setInt("boxes/" + mac_address + "/sound_request", 0);
      ...
      attachInterrupt(digitalPinToInterrupt(motion), detect_Motion, FALLING);
      break;
...
void loop() {
  ...
  if (setups == 0) {
    ...
    if (int(firebase.connect()) == 1) {
      mute_buzzer     =  firebase.getInt("boxes/" + mac_address + "/mute_buzzer");
      mode_buzzer     =  firebase.getInt("boxes/" + mac_address + "/mode_buzzer");
      sound_request   =  firebase.getInt("boxes/" + mac_address + "/sound_request");
      cb_temp         =  firebase.getInt("boxes/" + mac_address + "/cb_temp");
      cb_temp_buzzer  =  firebase.getInt("boxes/" + mac_address + "/cb_temp_buzzer");
      cb_motion       =  firebase.getInt("boxes/" + mac_address + "/cb_motion");
    ...
    if (digitalRead(water1) == LOW && digitalRead(water2) == LOW) {
      water_sensor = 1;
    } else {
      water_sensor = 0;
    }
    ...
    Serial.println("SoundRequest : " + String(sound_request));
    Serial.println("Cbtemp       : " + String(cb_temp));
    Serial.println("Cbmotion     : " + String(cb_motion));
    Serial.println("Setinterval  : " + String(setinterval));
    Serial.println("Latitude     : " + latitude);
    Serial.println("Longitude    : " + longitude);
    Serial.println("Water1       : " + String(digitalRead(water1)));
    Serial.println("Water2       : " + String(digitalRead(water2)));
    Serial.println("water_sensor : " + String(water_sensor));
    Serial.println("----------------------------");

    if ((runs == true) && (sound_request == 0) && (water_sensor == 1) && (mode_buzzer == 1) && (mute_buzzer == 0)) {
      Serial.println("************************ Water Alert ************************");
      BuzzerOn();
    }
    if ((runs == true) && (sound_request == 0) && ( temperatureC > cb_temp_buzzer) && (mode_buzzer == 1) && (mute_buzzer == 0)) {
      Serial.println("************************ Temperature Alert ************************");
      BuzzerOn();
    }
    if ((runs == true) && (sound_request == 0) && (motion_sensor == 1) && (mode_buzzer == 1) && (mute_buzzer == 0)) {
    ...
    // case send car state 1
    if (runs == true && sound_request == 0 && around_state == false) {
      around_state = true;
      unsigned long currentMillis1 = millis();
      if (currentMillis1 - previousMillis1 >=  60 * setinterval) {
        Serial.println("****************************");
        Serial.println("Car stop case car On");
        Serial.println("****************************");
        if (int(firebase.connect()) == 1) {
          firebase.setInt("boxes/" + mac_address + "/lastVehicle", 1);
          delay(100);
          around_state = false;
        }
        firebase.close();
    ...
    // case send car state 0
    if (runs == false && sound_request == 0 && around_state == false) {
      around_state = true;
      unsigned long currentMillis2 = millis();
      if (currentMillis2 - previousMillis2 >=  60 * setinterval) {
        Serial.println("****************************");
        Serial.println("Car stop case car default");
        Serial.println("****************************");
        if (int(firebase.connect()) == 1) {
          firebase.setInt("boxes/" + mac_address + "/lastVehicle", 0);
          delay(100);
          around_state = false;
        }
        firebase.close();
    ...
    // case send water state 1
    if (runs == true && sound_request == 0 && water_sensor == 1 && around_state == false) {
      around_state = true;
      unsigned long currentMillis3 = millis();
      if (currentMillis3 - previousMillis3 >=  60 * setinterval) {
        Serial.println("****************************");
        Serial.println("Car stop case water On");
        Serial.println("****************************");
        if (int(firebase.connect()) == 1) {
          firebase.setInt("boxes/" + mac_address + "/lastWater", 1);
          delay(100);
    ...
    // case send water state 0
    if (runs == true && soundRequest == 0 && water_sensor == 0 && around_state == false) {
      around_state = true;
      unsigned long currentMillis4 = millis();
      if (currentMillis4 - previousMillis4 >=  60 * setinterval) {
        Serial.println("****************************");
        Serial.println("Car stop case water default");
        Serial.println("****************************");
        if (int(firebase.connect()) == 1) {
          firebase.setInt("boxes/" + mac_address + "/lastWater", 0);
          delay(100);
          firebase.setStr("boxes/" + mac_address + "/lastLatitude",  latitude);
          firebase.setStr("boxes/" + mac_address + "/lastLongitude", longitude);
          delay(100);
    ...
    // case send temperature state 0
    if (runs == true && sound_request == 0 && temperatureC > cb_temp && around_state == false) {
      around_state = true;
    ...
    // case send temperature state 1
    if (runs == true && sound_request == 0 && temperatureC < cb_temp && around_state == false) {
      around_state = true;
      unsigned long currentMillis6 = millis();
      if (currentMillis6 - previousMillis6 >=  60 * setinterval) {
        Serial.println("****************************");
        Serial.println("Car stop case temperature default");
        Serial.println("****************************");
        if (int(firebase.connect()) == 1) {
          firebase.setFloat("boxes/" + mac_address + "/lastTemperature", 0);
          delay(100);
          around_state = false;
        }
        firebase.close();
    ...
    // case send motion state 1
    if (runs == true && sound_request == 0 && motion_sensor == 1 && around_state == false) {
      around_state = true;
      unsigned long currentMillis7 = millis();
      if (currentMillis7 - previousMillis7 >=  60 * setinterval) {
        Serial.println("****************************");
        Serial.println("Car stop case motion On");
        Serial.println("****************************");
        if (int(firebase.connect()) == 1) {
