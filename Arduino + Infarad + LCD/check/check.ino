...
int counter1 = 0;  // กำหนดตัวแปลสำหรับเอาไว้รอนับจำนวน
int counter2 = 20;                        // กำหนดตัวแปลสำหรับเอาไว้รอนับจำนวนที่ว่าง
...
void IntSwitchplus(void) { //ฟังชั่น บวกเลข
  if ((millis() - time_ms_buffer) > 1000) { // รันเวลา เอามา - กับเวลาที่กำหนด ถ้าน้อยกว่า 1000 หรือ การหน่วงประมาณ 3 วินาที เมื่อเข้าเงื่อนไงนี้ให้ทำบรรทัดช้าล่างต่อ
    ...
    if (counter1 >= 20) {                    // เลข 10 เอาไว้ set จำนวนที่นั่ง ตอนปกติ คือ 0 ที่นั่ง เมื่อขึ้น จะ +1 จนครบ 10 จะเห็นว่า เมื่อไหร่ที่ค่า  counter มากว่า 10 หมายถึงเกิน 10 ให้ทำเงื่อนไขข้างล่างต่อ
      counter1 = 20;                         // คือการ set ค่าให้ counter นั่น เท่ากับ 10 ไปเลย
    }
...
void IntSwitchwanting(void) { //ฟังชั่น ลบเลข
  if ((millis() - time_ms_buffer) > 1000) { // รันเวลา เอามา - กับเวลาที่กำหนด ถ้าน้อยกว่า 1000 หรือ การหน่วงประมาณ 3 วินาที เมื่อเข้าเงื่อนไงนี้ให้ทำบรรทัดช้าล่างต่อ
    ...
    if (counter2 >= 20) {                    // เลข 10 เอาไว้ set จำนวนที่นั่ง ตอนปกติ คือ 0 ที่นั่ง เมื่อขึ้น จะ +1 จนครบ 10 จะเห็นว่า เมื่อไหร่ที่ค่า  counter มากว่า 10 หมายถึงเกิน 10 ให้ทำเงื่อนไขข้างล่างต่อ
      counter2 = 20;                         // คือการ set ค่าให้ counter นั่น เท่ากับ 10 ไปเลย
    }
